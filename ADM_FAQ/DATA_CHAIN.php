...
$columns = array(
	...
	$datatable->set_order_by("date_edit");
...
if (isset($_POST['bab_cari'])) {
	...
	if ($pertanyaan_cari_!="")	{
		$data_pertanyaan_cari		="WHERE pertanyaan_faq LIKE ?";
		$prepare_pertanyaan_cari	= array("pertanyaan_faq" => $pertanyaan_cari_);
		$prepared_data 				= array_merge($prepared_data,$prepare_pertanyaan_cari);
		}
	if ($jawaban_cari_!="")	{
		$data_jawaban_cari		="AND jawaban_faq LIKE ?";
		$prepare_jawaban_cari	= array("jawaban_faq" => $jawaban_cari_);
		$prepared_data 			= array_merge($prepared_data,$prepare_jawaban_cari);
		}
	if ($daskum_cari_!="")	{
		$data_das_cari			="AND daskum_faq LIKE ?";
		$prepare_das_cari		= array("daskum_faq" => $daskum_cari_);
		$prepared_data 			= array_merge($prepared_data,$prepare_das_cari);
		}
	if ($_POST["bab_cari"]!="")	{
		$strCari="$strCari AND kode_bab=?";
		$prepare_kode_bab_cari 	= array("td_peraturan_bravo.jenis_per" => $_POST["bab_cari"]);
...
$data_pertanyaan_cari $data_jawaban_cari $data_das_cari $strCari",$columns,$prepared_data);
	...
	$ResultData[] = "<div class='pull-center'>".$datatable->number($i)."</div>".
                    ...
                    "<input type='hidden' id='pertanyaan-value-".$value->id."' value='".$value->pertanyaan_faq."'>".
                    "<input type='hidden' id='jawaban-value-".$value->id."' value='".$value->jawaban_faq."'>".
                    "<input type='hidden' id='daskum-value-".$value->id."' value='".$value->daskum_faq."'>".
					"<input type='hidden' id='ndver-value-".$value->id."' value='".$value->daskum_verifikasi."'>".
		...
		"<div class='col-md-12 row'>
			...
			<li class='time-label'>
				<div class='pull-right'>
					...
					".date("d-m-Y", strtotime($value->date_edit))."
					</span>
				</div>
			...
			<div class='timeline-item' style='margin-bottom:-39px'>
				...
				<div class='timeline-footer'>
					<div data-toggle='collapse' data-parent='#accordion' data-target='#collapse".$value->id."'>
						...
						<div class='pull-right'>".$value->user_input." || ".date("d-m-Y H:i:s", strtotime($value->date_edit))."</div>
					</div>
					<div id='collapse".$value->id."' class='panel-collapse collapse box-primary'>
					". nl2br($value->daskum_faq,true)."
					</div>
				</div>
			</div>
			</li>
