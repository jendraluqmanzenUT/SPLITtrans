...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$sandi 				= injection(trim($_POST['sandi']));
		$password 			= $_POST['password'];
		$salt 				= $_POST['salt'];
		$telp_input			= $_POST['telp'];
		$qoute_input		= "Bea Cukai Makin Baik";
		...
		$sqlkantorII 	= "SELECT kode_es2,strataII FROM td_daftar_kantor WHERE kode_es2='$strataII_input'";
		$datakantorII	= mysql_query( $sqlkantorII) or die(mysql_error());
		while($rowII=mysql_fetch_array($datakantorII)){
		$eselon2_detail_input	= $rowII['strataII'];
		}
		/*INSERT DETAIL STRATA III*/
		$sqlkantorIII 	= "SELECT kode_es3,strataIII FROM td_daftar_kantor WHERE kode_es3='$strataIII_input'";
		$datakantorIII	= mysql_query( $sqlkantorIII) or die(mysql_error());
		while($rowIII=mysql_fetch_array($datakantorIII)){
		$eselon3_detail_input	= $rowIII['strataIII'];
		}
		/*INSERT DETAIL STRATA IV*/
		$sqlkantorIV 	= "SELECT kode_es4,strataIV FROM td_daftar_kantor WHERE kode_es4='$strataIV_input'";
		$datakantorIV	= mysql_query( $sqlkantorIV) or die(mysql_error());
		while($rowIV=mysql_fetch_array($datakantorIV)){
		$eselon4_detail_input	= $rowIV['strataIV'];
		}
		/*INSERT ALAMAT KANTOR*/
		$sqlkantorV 		= "SELECT kode_es4,alamat_kantor,telp_kantor FROM td_daftar_kantor WHERE kode_es4='$strataIV_input'";
		$datakantorV		= mysql_query( $sqlkantorV) or die(mysql_error());
		while($rowV=mysql_fetch_array($datakantorV)){
		$alamat_kantorINP	= $rowV['alamat_kantor'];
		}
		/*INSERT TELP KANTOR*/
		$sqlkantorVI 		= "SELECT kode_es4,alamat_kantor,telp_kantor FROM td_daftar_kantor WHERE kode_es4='$strataIV_input'";
		$datakantorVI		= mysql_query( $sqlkantorVI) or die(mysql_error());
		while($rowVI=mysql_fetch_array($datakantorVI)){
		$telp_kantorINP		= $rowVI['telp_kantor'];
		}
		
		$tipologi_t	= $tipologi_input;
		if ($tipologi_t =="Kantor Pusat") { 
			$nama_kantor_input =$eselon2_detail_input;
		} elseif ($tipologi_t =="Kantor Wilayah/Kantor Pelayanan Utama") {
			$nama_kantor_input=$eselon2_detail_input;
		} else {
	...
	$sql_cont = "SELECT COUNT(*) FROM (SELECT * FROM users WHERE nama = '$nama_pegawai' OR nip = '$nip_pegawai' OR username = '$username') AS D"; 
	$result = $pdoMYSQL->prepare($sql_cont); 
	$result->execute(); 
	...
	if ($number_of_rows=="0") {
		$sql		= $pdoMYSQL->prepare("insert INTO users (
			...
			if ($sql->execute()) {
				...
				if($sql_od->execute()) {
					$sql1 = $pdoMYSQL->prepare("UPDATE users_register_acc SET 
					update_acc = :update_acc 
					WHERE username=:username");
					$sql1->bindParam(':username', $username);
					$sql1->bindParam(':update_acc', $notif_time);
					$sql1->execute();
					$sqlcek = $pdoMYSQL->prepare("SELECT * FROM users_register WHERE id=:id");
					$sqlcek->bindParam(':id', $id);
					$sqlcek->execute();
					$data = $sqlcek->fetch();			
					$sql2 = $pdoMYSQL->prepare("DELETE FROM users_register WHERE id=:id");
					...
					if ($sql2->execute()) {				
						...
						try {
							...
							$mail->isHTML(true);
							$mail->Subject = 'Pemberitahuan Permohonan Akun SPLIT';
							$mail->Body = 
							'
							<span style="font-family:source-sans-pro, Arial, sans-serif; font-size:12px;">
							Salam Bravo Bea Cukai,
							<br>
							Terima Kasih telah melakukan pengajuan aktivasi user SPLIT (Sistem Pendukung Layanan Informasi Terpadu).
							<br><br>
							Akun SPLIT Anda berhasil direvifikasi. silahkan login dengan username dan password yang telah Anda daftarkan sebelumnya.
							<br>
							...
							<span style="font-family:source-sans-pro, Arial, sans-serif; font-size:10px; font-style:italic; color:#8E8E8E">
							<b>PENTING</b><br>
						...
						$response = array(
							'status'=>'sukses',
							'pesan'=>'Data Berhasil Diregister',
							'html'=>$html
						);
					} else {
						$response = array(
					...
					$response = array(
						'status'=>'gagal',
						'pesan'=>'Data Gagal Diaktivasi [ SQL SERVER ]',
					);
				}
			} else {
				$response = array(
					'status'=>'gagal',
					'pesan'=>'Data Gagal Diaktivasi [ MYSQL SERVER ]',
				);
			}
	} else {
		$response = array(
			'status'=>'gagal',
			'pesan'=>'Data Pegawai Sudah Ada',
		);
	}
