$(document).ready(function(){
	...
	$('#form-pegawai').bootstrapValidator({
		...
		fields: {
				...
				nip_pegawai: {
					validators: {
						...
						numeric: {
							message: 'Nama hanya berisi angka saja'
						}
					}
				},
				email: {
					...
					regexp: {
							regexp: /^([a-z0-9\+_\-]+).+@(customs|kemenkeu)\.go.id$/,
							message: 'Email hanya menggunakan email customs.go.id dan kemenkeu.go.id'
						}
					}
				},
				username: {
					validators: {
						...
						regexp: {
							regexp: /^[a-zA-Z.]*$/,
							message: 'Password harus berisi Alphabet, Angka dan Karakter Titik saja'
						}
					}
				},
				password: {
					validators: {
						...
						stringLength: {
							min: 6,
							max: 20,
							message: 'Password minimal 6 digit dan maksimal 20 digit'
                    	},
						identical: {
                        	field: 'confirmpwd',
                        	message: 'Silahkan konfirmasi password Anda'
                    	},
						regexp: {
							regexp: /(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}/,
							message: 'Password harus memiliki huruf kecil, huruf besar dan angka'
						}
					}
				},
				confirmpwd: {
					validators: {
						...
						identical: {
                        	field: 'password',
                        	message: 'Password Anda tidak sama pastikan kembali'
                    	},
					}
				},
				telp: {
				...
				tipologi: {
					validators: {
						notEmpty: {
							message: 'Jenis tipologi kantor harus dipilih'
						},
					}
				},
				qoute: {
				...
				strataII: {
					validators: {
						notEmpty: {
							message: 'Jenis esselon kantor harus dipilih'
						},
					}
				},
				strataIII: {
				...
				jabatan: {
					validators: {
						notEmpty: {
							message: 'Jabata pegawai harus dipilih'
						},
					}
				},
				pangol: {
					validators: {
						notEmpty: {
							message: 'Pangkat dan golongan pegawai harus dipilih'
						},
					}
				},
			 },
	...
	$('#btn-simpan').click(function() {
	  	$('#form-pegawai').bootstrapValidator('validate');
   		if ($('#form-pegawai').bootstrapValidator('validate').has('.has-error').length) {
			new PNotify({
				title: 'Oh No!',
		...
		var data = new FormData();
			...
			$("#loading-simpan").show();
			$.ajax({
				url: '_ADMINISTRASI/Adm_Pegawai/PROSES_SIMPAN.php',
				type: 'POST',
				data: data,
				processData: false,
				contentType: false,
	...
	$('#btn-ubah').click(function() {
		...
		var data = new FormData();
			...
			$("#loading-ubah").show();
			$.ajax({
				url: '_ADMINISTRASI/Adm_Pegawai/PROSES_UBAH.php',
				type: 'POST',
				data: data,
				processData: false,
				contentType: false,
	...
	$("#btn-hapus").click(function(){
		...
		$("#loading-hapus").show();
		$.ajax({
			url: '_ADMINISTRASI/Adm_Pegawai/PROSES_HAPUS.php',
			type: 'POST',
			data: data,
			processData: false,
			contentType: false,
	...
	$("#btn-active").click(function(){
		...
		$("#loading-simpan").show();
		$.ajax({
			url: '_ADMINISTRASI/Adm_Pegawai/PROSES_ACTIVE.php',
			type: 'POST',
			data: data,
			processData: false,
			contentType: false,
			...
			success: function(response){ 
				...
				$("#loading-simpan").hide(); 
				$("#view_search").html(response.html);
				$("#modal-active").modal('hide'); // Close / Tutup Modal Dialog
			},
			error: function (xhr, ajaxOptions, thrownError) { // Ketika terjadi error
				alert("ERROR : "+xhr.responseText); // Munculkan alert
			}
	...
	$("#btn-disable").click(function(){
		...
		$("#loading-simpan").show();
		$.ajax({
			url: '_ADMINISTRASI/Adm_Pegawai/PROSES_DISABLE.php',
			type: 'POST',
			data: data,
			processData: false,
			contentType: false,
	...
	$('#modal-pegawai').on('hide.bs.modal', function (){	
		...
		$("#strataIII").val('').trigger('change');
		$("#strataIV").val('').trigger('change');
		$('#form-pegawai').bootstrapValidator('resetForm', true);
		$('select[name=tipologi]').children('option:last').remove();
