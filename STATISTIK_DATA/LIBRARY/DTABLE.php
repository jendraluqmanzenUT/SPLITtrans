...
class DTable extends Database
{
    ...
    {
        $result = $this->custom_query($sql,$prepare_data);
        $countdata1 = count($result->fetchAll());
		
        //total filtered default
        $this->record_total = $countdata1;
    }
    ...
    {
        $result = $this->custom_query($sql,$prepare_data);
        $countdata2 = count($result->fetchAll());
        //total filtered default
        $this->total_filtered = $countdata2;
...
//echo $do_order;        
        if (!empty($requestData['search']['value'])) {
            ...
            $sql .= " ORDER BY " . $do_order . "   " . $do_order_type . " OFFSET " . $requestData['start'] . " ROWS FETCH NEXT " . $requestData['length'] . " ROWS ONLY";
